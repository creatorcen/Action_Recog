...
def make_dataset(root, source):
    ...
    if not os.path.exists(source):
        print("Setting file %s for Smart City dataset doesn't exist." % (source))
        sys.exit()
    else:
        clips = []
...
def make_path(root,source,phase):
    csv_file = csv.reader(open(source))
    final_detail = []
    base_dir = root +'/'+phase

    for item in csv_file:
        singl_dir = os.path.join(base_dir,item[0])
...
def ReadSegmentRGB(path, offsets, new_height, new_width, new_length, is_color, name_pattern):
    ...
    for offset_id in range(len(offsets)):
        ...
        for length_id in range(1, new_length + 1):
            ...
            frame_path = path + "/Frame/" + frame_name
    ...
    for offset_id in range(len(offsets)):
        ...
        for length_id in range(1, new_length + 1):
            ...
            if cv_img_origin_x is None or cv_img_origin_y is None:
                print("Could not load file %s or %s" %
                      (frame_path_x, frame_path_y))
                sys.exit()
                # TODO: error handling here
            if new_width > 0 and new_height > 0:
                cv_img_x = cv2.resize(
                    cv_img_origin_x, (new_width, new_height), interpolation)
                cv_img_y = cv2.resize(
                    cv_img_origin_y, (new_width, new_height), interpolation)
            else:
...
class smcity(data.Dataset):
    ...
    def __init__(self,
        ...
        classes, class_to_idx = find_classes(root+'/'+phase)
        # OK the index of the train dataset
        # clips = make_dataset(root+'/'+phase, source)
        clips = make_path(root, source, phase)
            ...
            if self.phase == "train":
                if average_duration >= self.new_length:
                    offset = random.randint(
                        0, average_duration - self.new_length)
